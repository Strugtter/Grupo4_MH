...
async function data() {
  try {
    let eventos = await fetch(url);
    eventos = await eventos.json();
    eventos = eventos.response;
    for (let i = 0; i < eventos.length; i++) {
      let name = eventos[i].name;
    let image = eventos[i].image;
    let description = eventos[i].description;
    let price = eventos[i].price;
    let categ = eventos[i].category;
    let dat = eventos[i].date;
    let pla = eventos[i].place;
    let capa = eventos[i].capacity;
    let assis = eventos[i].assistance;
    }
